...
The demo lets you run a real differential attack in the browser, ordered so the intuition comes before the payoff. The **Differential Trace** tab (the default landing tab) visualizes how XOR differences propagate through the cipher step by step: at every sub-stage it shows both concrete states, their XOR difference, and an explicit marker labelling whether the difference is preserved (XOR-K rows), changed (S-box rows), or relocated (permute rows). Stepping onto a permute row animates the actual bit routing, drawing a connector from each active bit's source position to its destination and confirming the count of active bits is invariant. Once you have watched a difference survive the key XOR and mutate only at the S-box, the **Live Attack** tab lets you exploit exactly that fact: choose two plaintext values (hex bytes), collect 100–1000 ciphertext pairs from the toy SPN cipher, and execute last-round key recovery — the attack ranks all 256 candidate subkeys by bias count and identifies the correct one. The target differential shown here is auto-derived from the real cipher on load (not a magic constant) and annotated with the bias that makes the attack work. The **S-box Analysis** tab displays the full 16×16 Difference Distribution Table with clickable cells showing count, probability, and exploitability for each differential, plus a two-lane view making explicit that the single-nibble DDT drives an attack that recovers the full 8-bit last-round subkey spanning two S-box copies.
