 ...
 *    files the node under `incomplete`. The success, failure and insight
 *    callouts here all sit on 135deg linear washes of their own semantic
 *    colour (`rgba(0, 216, 111, 0.1)`, `rgba(255, 59, 92, 0.05)` and friends),
 *    so their text composites onto exactly what axe will not measure. That is
 *    also what the gate this replaced tried to hand-roll — with a regex over
 *    `backgroundImage` that returned a passing 100 whenever it matched nothing.
 ...
 *    `opacity: .85`, un-revealed trace rows at `0.35`, and several stage
 *    decorations between `0.4` and `0.85`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.trace-stages`
 *    and the DDT container are horizontal scrollers, and at 380px much of a
 *    16x16 DDT or a four-column trace row is scrolled out of view. Content past
 ...
 *    behind it and fall through to white, inventing a failure. Skip it, and
 *    rely on the wider viewport where the same cell is measured for real.
 *
 ...
 *    draws real SVG: the permute-stage wire overlay is built with
 *    `createElementNS` and animates `stroke-dashoffset`. An `<svg>` draws a
 ...
 *    ancestor-only walk would composite those labels onto the svg's own
 *    transparent background and report a ratio nothing on screen has. Note the
 *    underlay deliberately ignores stroke-only geometry such as `<line>`: SVG's
 *    initial `fill` is black and `getComputedStyle` reports that even for a
 *    line, so compositing one invents an opaque black backdrop under whatever
 *    it crosses.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here — and the old gate proved
     * it, because that is exactly what it used. The shared header mixes this
     * lab's `--accent` into its surfaces with `color-mix()`, the S-box cells are
     * painted from a computed `hsl()` ramp, and Chromium reports both to
     ...
     *
     * This page renders a 256-cell DDT, a 16-cell S-box grid and twelve trace
     * rows of bit dots, and every one of them walks the same handful of
     * ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // inactive tab panels and its results section with `display: none`, which
      // the test above already catches, so this line is silent here.
      ...
      // skip links here — the shared header's `.cl-skip-link` and the lab's own
      // `.skip-link` both park off the top edge and slide into view only on
    ...
    /**
     * `aria-hidden` text is removed from the accessibility tree, and this
     * arithmetic oracle exists to
     ...
     * that covers the start-here banner's arrow glyph and the un-revealed trace
     * rows. NOTE this is a deliberate narrowing, not a coverage hole: axe's own
     * `color-contrast` rule keys off VISUAL visibility, not the accessibility
     * tree, so it still measures `aria-hidden` text that paints on screen — and
     * it is what caught the `aria-hidden` DDT lane diagram this gate landed a
     * fix for. The two oracles cover different gaps; neither alone is enough.
     */
     ...
     *
     * This is not hypothetical in this fleet: interpolating a string ARRAY into
     * a template literal (`<g>${dots}</g>`) makes JS join it with commas,
     * leaving a run of stray "," text nodes inside the group, invisible on
     * screen but measurable by a naive walk. This lab builds its permute-wire
     * overlay with `createElementNS`, so the same shape is reachable here.
     */
